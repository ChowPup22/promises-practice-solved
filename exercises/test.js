...
const getLoginList = async (data) => {
  // Your code goes here...
  const allUsers = fetch(usersUrl)
    .then((res) => res.json())
    .then((json) => {
      return json.filter(user => data.includes(user.login))
    })
    .then((data) => {
      let arr = []
      data.forEach((user) => arr.push(user.login))
      return arr;
    })
    .catch(e => console.log(e));

  // const matchedUsers = allUsers.filter(user => data.includes(user.login));
...
const result = getData
  .then((res) => res.json())
  .then((json) => {
    return json.map((user) => user.login)
  })
  .then((arr) => getLoginList(arr))
